...
export function generateAgendaSeoHead({ post }) {
  const title = post.title
    ? `${post.title} — Agenda Netti Architetti`
    : 'Agenda — Netti Architetti';

  const rawDesc = post.content
    ? post.content.replace(/[#*`[\]()]/g, '').substring(0, 155)
    : `${post.title} — Leggi l'articolo sul blog di Netti Architetti, studio di architettura a Bari.`;
  const description = rawDesc.trim();

  const coverFile = post.files?.find(f => f.type === 'cover');
  const image = coverFile?.preview || DEFAULT_IMAGE;

  const pageUrl = `${BASE_URL}/agenda/${post.slug || post.id}`;

  const breadcrumbs = [
    { name: 'Home', url: BASE_URL },
    { name: 'Agenda', url: `${BASE_URL}/agenda` },
    { name: post.title || 'Articolo', url: pageUrl },
  ];
  ...
  const articleSchema = {
    ...
    ...(post.date && { dateModified: post.date }),
  };
    ...
    script: [
      {
        type: 'application/ld+json',
        innerHTML: JSON.stringify(
          buildWebPageSchema({ url: pageUrl, title, description, image, breadcrumbs, type: 'WebPage', datePublished: post.date, dateModified: post.date })
        ),
      },
